...
class ExteriorChamferTest(ScadTestCase):
    ...
    def test_sizes_side1(self):
        ...
        """
        # Positive radius.
        factory = ExteriorChamferFactory(skew_length=5.0, side=1)

        # Sharp angle.
        self.assertAlmostEqual(2.7060, factory.offset1(inner_angle=45.0), places=4)
        self.assertEqual(0.0, factory.offset2(inner_angle=45.0))

        # Oblique angle.
        self.assertAlmostEqual(6.5328, factory.offset1(inner_angle=135.0), places=4)
        ...
        # Zero skew length.
        factory = ExteriorChamferFactory(skew_length=0.0, side=1)
    ...
    def test_sizes_side2(self):
        ...
        """
        # Positive radius.
        factory = ExteriorChamferFactory(skew_length=5.0, side=2)

        # Sharp angle.
        self.assertEqual(0.0, factory.offset1(inner_angle=45.0))
        self.assertAlmostEqual(2.7060, factory.offset2(inner_angle=45.0), places=4)

        # Oblique angle.
        self.assertEqual(0.0, factory.offset1(inner_angle=135.0))
        self.assertAlmostEqual(6.5328, factory.offset2(inner_angle=135.0), places=4)
        ...
        # Zero skew length.
        factory = ExteriorChamferFactory(skew_length=0.0, side=2)
        self.assertEqual(0.0, factory.offset1(inner_angle=45.0))
        self.assertEqual(0.0, factory.offset2(inner_angle=315.0))
    ...
    def test_skew_height(self):
        ...
        """
        factory = ExteriorChamferFactory(skew_height=5.0, side=2)

        # Sharp angle.
        inner_angle = 45.0

        p1 = Vector2(0.5 * factory.skew_length(inner_angle=inner_angle), 0.0)
        p2 = p1 + Vector2.from_polar_coordinates(factory.offset2(inner_angle=inner_angle),
                                                 90.0 + 0.5 * (180.0 - inner_angle))
        ...
        profile = factory.create_smooth_profile(params=SmoothProfileParams(inner_angle=inner_angle,
                                                ...
                                                child=Empty())
        self.assertIsInstance(profile, ExteriorChamfer)
    ...
    def test_exterior_chamfer_pos(self) -> None:
        ...
        factories = [ExteriorChamferFactory(skew_length=5.0, side=2),
                     ExteriorChamferFactory(skew_height=3.0, side=1),
                     ExteriorChamferFactory(skew_height=3.0, side=2),
                     ExteriorChamferFactory(skew_length=5.0, side=1)]
    ...
    def test_exterior_chamfer_zero(self) -> None:
        ...
        factories = [ExteriorChamferFactory(skew_length=0.0, side=2),
                     ExteriorChamferFactory(skew_height=0.0, side=1),
                     ExteriorChamferFactory(skew_height=0.0, side=2),
                     ExteriorChamferFactory(skew_length=0.0, side=1)]
